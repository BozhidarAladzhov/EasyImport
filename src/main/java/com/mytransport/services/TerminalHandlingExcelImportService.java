...
public class TerminalHandlingExcelImportService {
    ...
    public void importPricesFromExcel (){
        ...
        try {
            InputStream inputStream = new ClassPathResource("data/TerminalHandlingDatabase.xlsx").getInputStream();
            Workbook workbook = new XSSFWorkbook(inputStream);
            Sheet sheet = workbook.getSheetAt(0);

            List<TerminalHandlingEntity> terminalHandlingEntityList = new ArrayList<>();
            ...
            for (int i = 1; i<= sheet.getLastRowNum(); i++){
                ...
                terminalHandlingEntityList.add(new TerminalHandlingEntity(origin, destination, vehicleType, price));

            }
            terminalHandlingRepository.deleteAll();
            terminalHandlingRepository.saveAll(terminalHandlingEntityList);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
